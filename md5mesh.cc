...
bool MD5Mesh::LoadMesh(char* str)
{
	...
	for (int i=0; i<iNumMeshes; i++)
	{
		...
		std::string sTmp;
		pFD >> sTmp;
		int iLastSlash = sTmp.find_last_of("/");
		int iLastQuote = sTmp.find_last_of("\"");
		iLastSlash += 1; //on veut le caractere juste apres le dernier slash
		sTmp.erase(iLastQuote);
		pMeshes[i].sNormal = "textures/"+sTmp.substr(iLastSlash);
		pMeshes[i].sColor = pMeshes[i].sNormal+"_color.bmp";
		pMeshes[i].sNormal += ".bmp";
		
		pMeshes[i].pMaterial = new Material ();
		Texture* tmpTex = new Texture ();
		tmpTex->LoadBMP((char*)pMeshes[i].sNormal.data(), TG_TEXTURE_2D);
		pMeshes[i].pMaterial->SetNormalTexture(tmpTex);
		tmpTex = new Texture();
		tmpTex->LoadBMP((char*)pMeshes[i].sColor.data(), TG_TEXTURE_2D);
